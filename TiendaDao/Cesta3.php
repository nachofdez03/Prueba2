...
function MostrarCesta()
{
    ...
    echo "<fieldset>";
    echo "<form name='f2' method='post' action='Cesta3.php' enctype='multipart/form-data'>";

    echo "<table border='2'>";

    echo "<th>Eliminar</th><th>Añadir Foto</th><th>Cod</th><th>Nombre</th><th>Cantidad</th><th>Precio</th>";
    ...
    foreach ($_SESSION['cesta'] as $clave => $valor) {
        ...
        echo "<td><input type='checkbox' name='Fotos[" . $clave . "]'></td>";
    ...
    echo "<th></th><th></th><th></th><th></th><th></th><th>Total</th>";
    ...
    echo "<input type='submit' name='AñadirFoto' value='Añadir Foto'>";
    ...
    if (isset($_POST['AñadirFoto']) && isset($_POST['Fotos'])) {

        $fotos = $_POST['Fotos'];

        foreach ($fotos as $clave => $valor) {  // Es la clave del producto

            $producto = $daoProd->obtener($clave);

            echo "Insertar foto para producto: " . $producto->__get("nombre");

            echo "Foto<input type='file' name='InsertarFoto'><br>";


            if ($_FILES['InsertarFoto']['tmp_name'] != "") {
                $temp = $_FILES['InsertarFoto']['tmp_name'];
                $foto = base64_encode(file_get_contents($temp));
            }
